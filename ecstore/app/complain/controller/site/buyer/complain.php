<?php
//投诉管理
class complain_ctl_site_buyer_complain extends b2c_ctl_site_member
{
	/**
	 * 构造方法
    ...
    public function main($nPage=1){
        $obj_complain=kernel::single('complain_buyer_complain');
        $aData=$obj_complain->get_list($_POST,$this,$nPage); 
        $this->pagedata['complain_list']=$aData['data'];
        
        //echo '<pre>';print_r($aData['data']);echo '</pre>';
        $this->pagination($nPage,$aData['page'],'main','','complain',$ctl='site_buyer_complain');
    ...
    public function show_comment($complain_id){
        ...
        $this->get_member_info($complain['from_member_id']);
    ...
    public function cancel_comment($complain_id){
        $data=array();
        $data['complain_id']=$complain_id;
        $data['status']='cancel';
        $data['last_modified']=time();
        $result=$this->mdl_complain->save($data);
        echo $result?$complain_id:'';
    }
    public function save_comment(){
        
        //echo '<pre>';print_r($_FILES);echo '</pre>';exit;
        $url = $this->gen_url(array('app' => 'complain','ctl' => 'site_buyer_complain', 'act'=>'show_comment','arg0'=>$_POST['complain_id']));
        $isBigImage=false;
    ...
    public function addSave(){
        ...
        $error_url = $this->gen_url(array('app' => 'complain','ctl' => 'site_buyer_complain', 'act'=>'add','arg0'=>$_POST['from_member_id'],'arg1'=>$_POST['order_id'],'arg2'=>$_POST['store_id']));
        $success_url=$this->gen_url(array('app'=>'complain','ctl'=>'site_buyer_complain','act'=>'main'));
        $isBigImage=false;
        foreach($_FILES['image']['size'] as $key=>$v){
            if(intval($v)>5242880){
                $isBigImage=true;
        ...
        $data['from_member_id']=$_POST['from_member_id'];
        $data['to_member_id']=$_POST['to_member_id'];
        $data['source']=$_POST['source'];
        $data['store_id']=$_POST['store_id'];
        $data['store_name']=$_POST['store_name'];
        $data['mobile']=$_POST['mobile'];
        $data['from_uname']=$_POST['from_uname'];
        $data['to_uname']=$_POST['to_uname'];
        ...
        $data_item['author_id']=$data['from_member_id'];
        $data_item['author']=$minfo['uname'];
        ...
        $result=$this->mdl_complain->save($data);
        if($result){
            $this->splash('success',$success_url,app::get('b2c')->_('投诉成功，请等待处理'),'','',false);
        }else{        
            $this->splash('failed',$error_url,app::get('b2c')->_('投诉失败'),'','',false);
            
        }  
       
    }
    public function add($member_id,$order_id,$store_id){
          $this->pagedata['order_id']=$order_id;
          $this->get_order_info($member_id,$order_id,$store_id);
    ...
    public function _output(){
        $this->action_view='buyer/complains/'.$this->action_view;
