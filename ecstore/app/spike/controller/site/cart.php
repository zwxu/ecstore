...
class spike_ctl_site_cart extends b2c_ctl_site_cart{
    ...
    public function addCheck($type='goods'){
        ...
        $arr_objects = array();
        if ($objs = kernel::servicelist('spike_cart_object_apps'))
        {
            foreach ($objs as $obj)
            {
      ...
      // $oCartGoods->no_database = true;
       ...
       if($type=='goods'){
           ...
           if(!$obj_ident = $oCartGoods->add_object($aData,$msg)){
                ...
                $this->redirect($this->gen_url( array('app'=>'spike','act'=>'checkout','ctl'=>'site_cart','arg1'=>1)));
           }
       }else{
    ...
    public function checkout($isfastbuy=0){
        ...
        $this->pagedata['shipping_url'] = $this->gen_url( array('app'=>'spike','act'=>'shipping','ctl'=>'site_cart') );
        $this->pagedata['total_url'] = $this->gen_url( array('app'=>'spike','act'=>'total','ctl'=>'site_cart') );
    ...
    public function checkout_result($isfastbuy=0)
    {
        ...
        $shop['url']['shipping'] = app::get('site')->router()->gen_url(array('app'=>'spike','ctl'=>'site_cart','act'=>'shipping'));
        $shop['url']['total'] = app::get('site')->router()->gen_url(array('app'=>'spike','ctl'=>'site_cart','act'=>'total'));
        ...
        $this->pagedata['updateStoreTotalUrl'] = app::get('site')->router()->gen_url(array('app'=>'spike','ctl'=>'site_cart','act'=>'updateStoreTotal'));

        $this->pagedata['aCart']['inAct'] = 'true';

        $this->page('site/cart/checkout.html',false,'spike');
    }
	...
	public function remove_store_coupon($store_id){
     ...
     public function _common($flag=0) {
        ...
        // 购物车数据项的render
        $this->pagedata['item_section'] = $this->mCart->get_spike_item_render();
	...
	public function loginBuy($isfastbuy=0)
    {
        ...
        if(!isset($_SESSION['next_page']))
            $_SESSION['next_page'] = $this->gen_url(array('app'=>'spike','ctl'=>'site_cart','act'=>'checkout'));
    ...
    public function updateStoreTotal(){
        ...
        $str_html = $this->fetch("site/cart/item/goods/business_total.html",'spike');
        echo $str_html;exit;
    }
