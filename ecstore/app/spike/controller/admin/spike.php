...
class spike_ctl_admin_spike extends desktop_controller{
    ...
    public function index(){
        $this->finder('spike_mdl_activity',array(
                'title'=>app::get('spike')->_('秒杀活动'),
                'actions'=>array(
                        array('label'=>app::get('spike')->_('发起秒杀活动'),'icon'=>'add.gif','href'=>'index.php?app=spike&ctl=admin_spike&act=add',
                        ),
                        array('label'=>app::get('b2c')->_('删除'),'submit'=>'index.php?app=spike&ctl=admin_spike&act=delete')
                    ),
    ...
    public function add(){
        $this->pagedata['from_submit_url'] = $this->router->gen_url(array('app'=>'spike','ctl'=>'admin_spike','act'=>'toAdd'));
    ...
    public function toAdd(){
        $this->begin($this->router->gen_url(array('app'=>'spike','ctl'=>'admin_spike','act'=>'index'))); 
        $data = $this->get_data();
    ...
    function getBusinessActivity(){
        ...
        $act_obj = $this->app->model('spikeapply');
    ...
    function delete(){
        $this->begin('index.php?app=spike&ctl=admin_spike&act=index');
