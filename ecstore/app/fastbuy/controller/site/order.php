...
class fastbuy_ctl_site_order extends b2c_ctl_site_order{
    var $noCache = true;
     ...
     public function create()
    {
        ...
        if ($login_type == 'href' && !$arrMember['member_id'] && $_COOKIE['S']['ST_ShopEx-Anonymity-Buy'] != 'true')
            $this->redirect(array('app'=>'b2c','ctl'=>'fastbuy_cart','act'=>'loginbuy','arg0'=>'1'));
		...
		if(!$_SESSION['S[Cart_Fastbuy]']['goods']){
			$this->end(false,'商品信息错误',$this->gen_url(array('app'=>'site','ctl'=>'default','act'=>'index')),true,true);
		}
		...
		$aCart['store_free_shipping'] = array_pop($aCart['store_free_shipping']);
        $aCart['subtotal_weight_store'] = array_pop($aCart['subtotal_weight_store']);


		//begin 验证码
		//echo "<pre>";print_r($aCart);exit;
		if($aCart['inAct']&&!base_vcode::verify('b2c',intval($_POST['verifycode']))){
			$this->end(false, '验证码不正确', 'javascript:changeimg("membervocde");',true,true);
        }else{
			unset($_POST['userVcode']);
			unset($_POST['verifycode']);
        ...
        $order_data['store_id'] = array_shift(array_keys($business_goods));
        ...
        if ($result)
        {
            ...
            //$this->cart_model->remove_object(); //立即购买不清购物车
