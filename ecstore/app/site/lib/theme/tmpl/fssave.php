...
class site_theme_tmpl_fssave 
{
    ...
    public function update($theme){
        ...
        $data = app::get('site')->model('themes_tmpl')->getList('*', array("theme"=>$theme));
        foreach ($data as $row){
            $sys_list[$row['tmpl_path']] = $row['tmpl_type'];
        }
        $this->__get_all_files(THEME_DIR . '/' . $theme, $list, false);
        $ctl = $this->get_name();
        foreach($list AS $key=>$value){
            if (pathinfo($value, PATHINFO_EXTENSION) != 'html') continue;
            if (strpos($value, '.bak_') !== false) continue;
            $file_name = basename($value, '.html');
            if(!strpos($file_name,'.')){
                if(($pos=strpos($file_name,'-'))){
                    $type=substr($file_name,0,$pos);
            ...
            if($type && array_key_exists($type, $ctl)){
                if (array_key_exists($file_name.'.html', $sys_list)) {
                    $sys_list[$file_name.'.html'] = '';
                } else {
                    $array = array(
                        'theme'     => $theme,
                        'tmpl_type' => $type,
        ...
        foreach ($sys_list as $tmpl_path => $sys_type) {
            if ($sys_type) {
                $this->delete_tmpl($tmpl_path, $theme, true);
            }
        }
    }
    ...
    public function delete_tmpl($tmpl, $theme, $force=false) 
    {
        $source = THEME_DIR . '/' . $theme . '/' . $tmpl;
        if(!$force && !is_file($source))   return false;
        $data = app::get('site')->model('themes_tmpl')->getList('*', array('theme'=>$theme, 'tmpl_path'=>$tmpl));
        if($data[0]['id'] > 0){
