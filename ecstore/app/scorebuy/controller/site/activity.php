...
class scorebuy_ctl_site_activity extends b2c_frontpage{
    ...
    function verify_member(){
     ...
     protected function output($app_id='scorebuy'){
    ...
    public function attend(){
        ...
        if($business_activity_cat){
            ...
            $this->pagedata['activity_tab_cur'] = 'scorebuy';
        }
        //加载活动tab end 
   ...
   public function toAttend($id){
        $url = $this->gen_url( array('app'=>'scorebuy','ctl'=>'site_activity','act'=>'attend') );
        $this->begin($url);
        $oActivity = $this->app->model('activity');
        $nowTime = time();
        $filter = array(
        ...
        $this->pagedata['return_url'] = $this->gen_url( array('app'=>'scorebuy','ctl'=>'site_activity','act'=>'get_goods_info') );
        $this->pagedata['submit_url'] = $this->gen_url( array('app'=>'scorebuy','ctl'=>'site_activity','act'=>'saveToAttend') );

        //加载会员等级信息 start
        $memLvObj = app::get('b2c')->model('member_lv');
        $memLvs = $memLvObj->getList('member_lv_id,name',array('display'=>'false'));
        $this->pagedata['memberLv'] = $memLvs;
        //加载会员等级信息 end
   ...
   public function myAttend(){
        $businessActivity = $this->app->model('scoreapply');
        ...
        foreach($busineeActivityInfo as $k=>$v){
            ...
            if($v['status'] == '2'){
                ...
                $busineeActivityInfo[$k]['goodsLink'] = $this->gen_url( array('app'=>'scorebuy','ctl'=>'site_product','act'=>'index','args'=>$args) );
            }else{
   ...
   public function editAttend($id){
        ...
        $busineeActivityInfo = $businessActivity->getList('*',$filter);
        if($store_id != $busineeActivityInfo[0]['store_id']){
            $this->end(false,'您没有权限编辑该活动！');
        }

        if(!$busineeActivityInfo || empty($busineeActivityInfo) || $store_id != $busineeActivityInfo[0]['store_id'] || $busineeActivityInfo[0]['status'] != 3){
        ...
        $memLvScores = $memLvScoreObj->getList('*',array('aid'=>$busineeActivityInfo['id'],'gid'=>$busineeActivityInfo['gid']));
   ...
   public  function quitActivity($id){
        ...
        $url = $this->gen_url( array('app'=>'scorebuy','ctl'=>'site_activity','act'=>'myAttend') );
        $this->begin($url);
        ...
        $business = $businessactivity->dump(array('id'=>$id),'gid,isMenLv,status');
        ...
        if($re){
            if($business['isMenLv'] == '1'){
                $memScoreObj = app::get('scorebuy')->model('memberlvscore');
                $result = $memScoreObj->delete(array('aid'=>$id,'gid'=>$business['gid']));
                if(!$result){
                    $this->end(false,'退出失败！');
                }
            }
     ...
     */
     public function saveToAttend(){
        ...
        //开启会员价格时检索积分
        if($data['isMemLv'] == '1'){
            foreach($data['memLvScore'] as $levelId=>$score){
                if($score == ''){
                   $this->end(false,'请输入会员积分',null,false,true);
                }
            }
        ...
        if($data['price'] >= $goods_arr['price']){
            $this->end(false,'活动价格不得高于原价',null,false,true);
        }
        ...
        if($rs){
            if($data['isMemLv'] == '1'){
                foreach($data['memLvScore'] as $levelId=>$score){
                    $item = array();
                    $item['aid'] = isset($data['id'])?$data['id']:$rs;
                    $item['gid'] = $data['gid'];
                    $item['level_id'] = $levelId;
                    $item['score'] = $score;
                    $item['price'] = $data['memLvPrice'][$levelId];
                    $item['last_price'] = $data['memLvPrice'][$levelId];

                    $result = $object->addMemLvScore($item);
    ...
    private function load_info(){
       #获取会员基本信息
        ...
        $store_info = $storemanger->dump(array('store_id'=>$store_id),'shop_name,store_region');
   ...
   /*
    ...
    function detailMemPrice(){
        $id = $_GET['id'];
        $gid = $_GET['gid'];
        $memberLvPriceObj = app::get('scorebuy')->model('memberlvscore');
        $memberLv = $memberLvPriceObj->getMemLvScoreByIds($id,$gid);
        if($memberLv){
            $this->pagedata['memberLvPrice'] = $memberLv;
        }

        $this->page('site/member/memPrice.html',true);
    }
