...
class scorebuy_ctl_site_product extends b2c_ctl_site_product{
    ...
    public function index() {
       ...
       $this->pagedata['specShowItems'] =$specValue;
        ...
        if ($aGoods['product']){ //如果商品有货品处理价格
            ...
            //修改团购价格
            $scoreapplyObj = $this->app->model('scoreapply');
            $scoreObj = $this->app->model('activity');
            $filter = array('gid'=>$this->id,'id'=>$act_id,'status'=>'2');
            $scoreapply = $scoreapplyObj->dump($filter,'*');
            //无活动时跳转至普通商品
            if(!$scoreapply || empty($scoreapply)){
                header('Location:'.$normalG_url);
                exit;
            }
            $activity = $scoreObj->dump(array('act_id'=>$scoreapply['aid']),'start_time,end_time,act_open');

            $this->pagedata['activity'] = $activity;
            $this->pagedata['actapply'] = $scoreapply;
            $aGoods['current_price'] = $scoreapply['last_price'];
            $aGoods['current_score'] = $scoreapply['score'];
            //活动会员价格
            if($mlv && $scoreapply['isMemLv'] == '1'){
                $memLvScoreObj = app::get('scorebuy')->model('memberlvscore');
                $memlvs = $memLvScoreObj->getMemLvScoreByIds($act_id,$gid);
                $aGoods['current_price'] = $memlvs[$this->site_member_lv_id]['last_price'];
                $aGoods['current_score'] = $memlvs[$this->site_member_lv_id]['score'];
            }
        ...
        $this->pagedata['goodshtml']['store'] = kernel::single('scorebuy_goods_detail_store')->show($gid,$aGoods);

        //积分会员价
        $this->pagedata['goodshtml']['mlv_price'] = kernel::single('scorebuy_goods_detail_mlvprice')->show($gid,$act_id);
        ...
        //页面基本信息  servicelist  
        $this->pagedata['info_page_list'] = $this->_get_servicelist_by('score_products_index_info');
        ///按钮  servicelist 
        $this->pagedata['btn_page_list'] = $this->_get_servicelist_by('score_products_index_btn');
    ...
    function joinscore(){
        $this->page('site/product/joinscore.html',true);
    }
