...
class scorebuy_ctl_site_cart extends b2c_ctl_site_cart{
    ...
    public function addCheck($type='goods'){
        ...
        $arr_objects = array();
        if ($objs = kernel::servicelist('score_cart_object_apps'))
        {
            foreach ($objs as $obj)
            {
        ...
        if($obj_isScoreEnough){
            if(!$obj_isScoreEnough->isScoreEnough($aData['goods']['goods_id'],$aData['goods']['num'],$member_id)){
                echo json_encode( array('status'=>'faile','error'=>'您的积分不足！') );exit;
            }
        }
      ...
      // $oCartGoods->no_database = true;
       ...
       if($type=='goods'){
           ...
           if(!$obj_ident = $oCartGoods->add_object($aData,$msg)){
                ...
                $this->redirect($this->gen_url( array('app'=>'scorebuy','act'=>'checkout','ctl'=>'site_cart','arg1'=>1)));
           }
       }else{
    ...
    {   $isfastbuy = 1;
        ...
        if($arr_args['get'][0] != 'score'){
            ...
            if (!$arrMember['member_id'] && (($_COOKIE['S']['ST_ShopEx-Anonymity-Buy'] != 'true') || $is_member_buy != 'true'))
                ...
                $this->redirect(array('app'=>'scorebuy','ctl'=>'site_cart','act'=>'loginBuy','arg0'=>'1'));
        }
        ...
        $this->pagedata['shipping_url'] = $this->gen_url( array('app'=>'scorebuy','act'=>'shipping','ctl'=>'site_cart') );
        $this->pagedata['total_url'] = $this->gen_url( array('app'=>'scorebuy','act'=>'total','ctl'=>'site_cart') );
    ...
    public function checkout_result($isfastbuy=0)
    {
        ...
        //计算商品积分
        $gid = $this->pagedata['aCart']['object']['goods'][0]['obj_items']['products'][0]['goods_id'];
        $gnum = $this->pagedata['aCart']['object']['goods'][0]['quantity'];
        $scoreapplyObj = app::get('scorebuy')->model('scoreapply');
        $aid = $scoreapplyObj->getOnActIdByGoodsId($gid);
        $apply = $scoreapplyObj->dump(array('id'=>$aid),'*');
        if($apply['isMemLv'] == '1'){
            $memLvScoreObj = app::get('scorebuy')->model('memberlvscore');
            $memLvs = $memLvScoreObj->getMemLvScoreByIds($aid,$gid);
            $subtotal_consume_score = $memLvs[$arrMember['member_lv']]['score'] * $gnum;
        }else{
            $subtotal_consume_score = $apply['score'] * $gnum;
        }
        $this->pagedata['goods_use_score'] = $subtotal_consume_score;
        //计算商品积分
        ...
        $shop['url']['shipping'] = app::get('site')->router()->gen_url(array('app'=>'scorebuy','ctl'=>'site_cart','act'=>'shipping'));
        $shop['url']['total'] = app::get('site')->router()->gen_url(array('app'=>'scorebuy','ctl'=>'site_cart','act'=>'total'));
        ...
        $this->pagedata['updateStoreTotalUrl'] = app::get('site')->router()->gen_url(array('app'=>'scorebuy','ctl'=>'site_cart','act'=>'updateStoreTotal'));;

        $this->page('site/cart/checkout.html',false,'scorebuy');
    }
	...
	public function remove_store_coupon($store_id){
     ...
     public function _common($flag=0) {
        ...
        // 购物车数据项的render
        $this->pagedata['item_section'] = $this->mCart->get_score_item_render();
	...
	public function loginBuy($isfastbuy=0)
    {
        ...
        if(!isset($_SESSION['next_page']))
            $_SESSION['next_page'] = $this->gen_url(array('app'=>'scorebuy','ctl'=>'site_cart','act'=>'checkout'));
    ...
    public function total(){
        $this->_common();
        $obj_total = new scorebuy_order_total();
        $sdf_order = $_POST;
        $arrMember = $this->get_current_member();
    ...
    public function updateStoreTotal(){
        ...
        $str_html = $this->fetch("site/cart/item/goods/business_total.html",'scorebuy');
        echo $str_html;exit;
    }
