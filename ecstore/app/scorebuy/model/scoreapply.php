<?php
class scorebuy_mdl_scoreapply extends dbeav_model{
    function getOnActIdByGoodsId($goods_id){
        $nowTime = time();
        $sql = "select a.id as act_id from sdb_scorebuy_scoreapply as a".
        " join sdb_scorebuy_activity as s on s.act_id=a.aid".
        " where a.gid={$goods_id} and a.status='2' and s.act_open='true' and {$nowTime} >= start_time and {$nowTime} <= end_time";
    ...
    function getNoEndActIdByGoodsId($goods_id){
        ...
        $sql = "select a.id as act_id from sdb_scorebuy_scoreapply as a".
        " join sdb_scorebuy_activity as s on s.act_id=a.aid".
        " where a.gid={$goods_id} and a.status='2' and s.act_open='true' and {$nowTime} <= end_time";
        $result = $this->db->selectrow($sql);
        if ($result && !empty($result)){
            return $result['act_id'];
        }else{
    ...
    function getOnActGIdByStatus($status){
        $nowTime = time();
        $sql = "select a.gid from sdb_scorebuy_scoreapply as a".
        " join sdb_scorebuy_activity as s on s.act_id=a.aid".
    ...
    function _filter($filter,$tbase=''){
    	...
    	if(isset($filter['actname']) && $filter['actname'] != ''){
    		$actObj = app::get('scorebuy')->model('activity');
