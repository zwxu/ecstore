...
class scorebuy_order_total
{
    ...
    public function payment_detail(&$controller,$cart,$sdf_order)
    {
        ...
        $subtotal_weight_store = $cart['subtotal_weight_store'];
        $subtotal_store_good_price = $cart['subtotal_store_good_price'];
        $store_free_shipping = $cart['store_free_shipping'];
        foreach((array)$sdf_order['shipping_id'] as $s_k=>$s_v){
            if($s_v)
            {
                $dlytype = $controller->app->model('dlytype');//配送方式
                $dlytype_info = $dlytype->dump($s_v,'*');
                if(is_array($sdf_order['is_protect'])){
                    if($sdf_order['is_protect'][$s_k] === 'true' || $sdf_order['is_protect'][$s_k] === '1' || $sdf_order['is_protect'][$s_k] === true){//配送设置了保价
                        if ($dlytype_info['protect'] == 'true')
                        {  
                            /** 保价费界定为商品的最原始价格 **/
                            $protect = $objMath->number_multiple(array($subtotal_store_good_price[$s_k], $dlytype_info['protect_rate']));
                            $store_cost_protect[$s_k] = $protect>$dlytype_info['minprice']?$protect:$dlytype_info['minprice'];//保价费
                        }
                    }
                }else{
                    if($sdf_order['is_protect'] === 'true' || $sdf_order['is_protect'] === '1' || $sdf_order['is_protect'] === true){//配送设置了保价
                        ...
                        {  
                            /** 保价费界定为商品的最原始价格 **/
                            $subtotal_store_good_price = array_shift($subtotal_store_good_price);
                            $protect = $objMath->number_multiple(array($subtotal_store_good_price, $dlytype_info['protect_rate']));
                            $store_cost_protect[$k] = $protect>$dlytype_info['minprice']?$protect:$dlytype_info['minprice'];//保价费
                ...
                if(is_array($store_free_shipping)){
                    $store_free_shipping_store = $store_free_shipping[$s_k];
                    $total_weight_store = $subtotal_weight_store[$s_k];
                }else{
                    $store_free_shipping_store = $store_free_shipping;
                    $total_weight_store = $subtotal_weight_store;
                }
                if($store_free_shipping_store === 0){
                    $store_cost_freight[$s_k] = 0;
                }else{

                    $store_cost_freight[$s_k] = @utils::cal_fee($dlytype_info['dt_expressions'],$total_weight_store,$cost_item,$dlytype_info['firstprice'],$dlytype_info['continueprice']);//配送费
                }
            }
        }
        if(count($store_cost_freight) > 1){
            $cost_freight = array_sum($store_cost_freight);
        }else{
            foreach((array)$store_cost_freight as $v){
                $cost_freight = $v;
            }
        }
        if(count($store_cost_protect) > 1){
            $cost_protect = array_sum($store_cost_protect);
        }else{
            foreach((array)$store_cost_protect as $v){
                $cost_protect = $v;
            }
        }
        //$cost_freight = count($cost_freight) > 1 ? array_sum($cost_freight) : array_pop($cost_freight);
        //$cost_protect = count($cost_protect) > 1 ? array_sum($cost_protect) : array_pop($cost_freight);
        /**
         * 订单免运费
         */
        if (isset($cart_info['is_free_shipping']) && $cart_info['is_free_shipping'])
        ...
        //begin
        foreach($cart['subtotal_store_good_price'] as $tk=>$tv){
            $cart['subtotal_store_good_price'][$tk] = $cart['subtotal_store_good_price'][$tk] + $store_cost_freight[$tk] + $store_cost_protect[$tk]- $cart['store_count'][$tk];
            if($cart['is_free_shipping'][$tk]==1){
                $cart['subtotal_store_good_price'][$tk] -= $store_cost_freight[$tk];
            }
            if($cart['subtotal_store_good_price'][$tk] < ０){
                $cart['subtotal_store_good_price'][$tk] = 0;
            }
        }
        $cost_total = 0;
        foreach($cart['subtotal_store_good_price'] as $tk=>$tv){
            $cost_total +=  $cart['subtotal_store_good_price'][$tk];
        }
        //$controller->pagedata['cost_total'] = $cost_total;
        ...
        if(is_array($sdf_order['shipping_id'])){
        ...
        {
            $over_real_point = $obj_point_dis->generate_total($sdf_order, $total_amount, $subtotal_consume_score, $total_point,$cost_freight);//增加参数$cost_freight(运费) 
        ...
        if ($sdf_order['member_id'])
        {
            ...
            $arr_member = $obj_members->dump($sdf_order['member_id'], 'point,member_lv_id');
            ...
            //商品使用积分
            $gid = $cart_info['object']['goods'][0]['obj_items']['products'][0]['goods_id'];
            ...
            if($apply['isMemLv'] == '1'){
                ...
                $subtotal_consume_score += $memLvs[$arr_member['member_lv']['member_group_id']]['score'] * $cart_info['object']['goods'][0]['quantity'];
                $goods_use_score = $memLvs[$arr_member['member_lv']['member_group_id']]['score'] * $cart_info['object']['goods'][0]['quantity'];
            }else{
                $subtotal_consume_score += $apply['score'] * $cart_info['object']['goods'][0]['quantity'];
            }
        }
        else
        {
        ...
        $payment_detail = array(
                        'goods_use_score'=>$goods_use_score,
                        'cost_item'=>$objCurrency->amount_nocur($cost_item, $sdf_order['cur'], false, false),
