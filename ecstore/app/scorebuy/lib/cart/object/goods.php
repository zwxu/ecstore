 ...
 */
class scorebuy_cart_object_goods extends b2c_cart_object_goods
{
    ...
    function check($gid,$pid,$quantity=0,&$msg){
        ...
        $arr = kernel::single('scorebuy_cart_process_goods')->checkgoods($gid);
        if(!$arr){
            return true;
        }
        ...
        $memberbuy = app::get('scorebuy')->model('memberbuy');
    ...
    public function check_store($arr_data, $arr_carts, &$msg='',$_check_adjunct=true)
    {
        ...
        if($_check_adjunct) {
            ...
            if( !$flag ) {
                ...
                foreach( kernel::servicelist('score_addtocart_check') as $object ) {
    ...
    private function _check($aData, $_check_adjunct=true) {
        ...
        if($_check_adjunct) {
            ...
            if( !$flag ) {
                ...
                foreach( kernel::servicelist('group_addtocart_check') as $object ) {
                    if( !is_object($object) ) continue;
                    $flag = $object->check( $goods_id,$arr_product['product_id'],($result[$arr_product['product_id']]?$result[$arr_product['product_id']]:$aData['quantity']),$msg );
                    if( !$flag ) return $this->get_error_msg( $msg );
                }
    ...
    function _get_products($aProductId) {
       ...
       foreach($aProduct as $row) {
           ...
           if($row['score_setting'] == 'percent'){
                ...
                $aid = $scoreapplyObj->getOnActIdByGoodsId($row['goods_id']);
                $apply = $scoreapplyObj->dump(array('id'=>$aid),'*');
                if($apply['isMemLv'] == '1'){
                    $memLvs = $memLvScoreObj->getMemLvScoreByIds($aid,$row['goods_id']);
                    $price = $memLvs[$this->arr_member_info['member_lv']]['last_price'];
                }else{
                    $price = $apply['last_price'];
                }
                $row['gain_score'] = $price * ($row['gain_score']/100) * $point_money_value;
           }
           if($price){
                $row['price'] = $price;
           }
