 ...
 */
class groupbuy_cart_object_goods extends b2c_cart_object_goods
{
    ...
    function check($gid,$pid,$quantity=0,&$msg){
        ...
        $arr = kernel::single('groupbuy_cart_process_goods')->checkgoods($gid);
        if(!$arr){
            return true;
        }
    ...
    function _get_products($aProductId) {
       ...
       foreach($aProduct as $row) {
           ...
           //修改团购价格
           if($row['act_type'] == 'group'){
                $groupObj = app::get('groupbuy')->model('groupapply');
