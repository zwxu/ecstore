<?php
class groupbuy_finder_groupapply{
    function __construct($app){
        $this->app = $app;
        $this->router = app::get('desktop')->router();
        $this->businessActivity = $this->app->model('groupapply');
    }//End
    ...
    function detail_basic($id){
        ...
        $render->pagedata['from_submit_url'] = $this->router->gen_url(array('app'=>'groupbuy','ctl'=>'admin_groupapply','act'=>'audit'));
        $render->pagedata['from_cancle_url'] = $this->router->gen_url(array('app'=>'groupbuy','ctl'=>'admin_groupapply','act'=>'audit','sign'=>'true'));
        ...
        $activity = app::get('groupbuy')->model('activity')->dump(array('act_id'=>$aid),'*');
        $activity['start_time'] = date('Y-m-d',$activity['start_time']);
        $activity['end_time'] = date('Y-m-d',$activity['end_time']);
        $activity['goodsLink'] = app::get('site')->router()->gen_url( array( 'app'=>'groupbuy','real'=>1,'ctl'=>'site_product','args'=>array($gid,'','',$arr['id']) ) );
    ...
    function detail_last_price($id){
        echo '<pre>';print_r($id);die;//qianleidebug
    }
