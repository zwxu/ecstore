<?php
class groupbuy_auto_pay {
    function exec_auto(){
        $payTime = app::get('b2c')->getConf('site.group.payed_time');
        ...
        foreach($orders as $k=>$v){
            if($this->docancel($v['order_id'])){
                ...
                if($apply['nums'] != ''){
                    ...
                    $sql = "update sdb_groupbuy_groupapply set remainnums=".($apply['remainnums']+$goodsnum['gnum']).' where id='.$v['act_id'];
    ...
    public function docancel($order_id){
        ...
        $sdf['opname'] = '定时脚本';
        $b2c_order_cancel = kernel::single("groupbuy_order_cancel");
