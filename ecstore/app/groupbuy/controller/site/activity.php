...
class groupbuy_ctl_site_activity extends b2c_frontpage{
    ...
    function verify_member(){
     ...
     protected function output($app_id='groupbuy'){
    ...
    public function attend(){
        ...
        if($business_activity_cat){
            ...
            $this->pagedata['activity_tab_cur'] = 'groupbuy';
        }
        //加载活动tab end 
   ...
   public function toAttend($id){
        $url = $this->gen_url( array('app'=>'groupbuy','ctl'=>'site_activity','act'=>'attend') );
        $this->begin($url);
        $oActivity = $this->app->model('activity');
        $nowTime = time();
        $filter = array(
        ...
        $this->pagedata['return_url'] = $this->gen_url( array('app'=>'groupbuy','ctl'=>'site_activity','act'=>'get_goods_info') );
        $this->pagedata['submit_url'] = $this->gen_url( array('app'=>'groupbuy','ctl'=>'site_activity','act'=>'saveToAttend') );
        $this->output();
       //$oActivity
   }
    /*
   ...
   public function myAttend(){
        $businessActivity = $this->app->model('groupapply');
        ...
        foreach($busineeActivityInfo as $k=>$v){
            ...
            if($v['status'] == '2'){
                ...
                $busineeActivityInfo[$k]['goodsLink'] = $this->gen_url( array('app'=>'groupbuy','ctl'=>'site_product','act'=>'index','args'=>$args) );
            }else{
   ...
   public function quitActivity($id){
        ...
        $url = $this->gen_url( array('app'=>'groupbuy','ctl'=>'site_activity','act'=>'myAttend') );
        $this->begin($url);
     ...
     */
     public function saveToAttend(){
        ...
        if($data['aid']){
            $actObj = app::get('groupbuy')->model('activity');
            $nowTime = time();
            $filter = array(
            ...
            $act = $actObj->dump($filter,'nums');
            if(!$act){
                $this->end(false,'活动不存在',null,false,true);
            }
            if($act['nums'] > 0 && $act['nums'] !='' && $data['nums'] != ''){
                if($data['nums'] < $act['nums']){
                    $this->end(false,'参加活动数量不能小于平台设置最低库存'.$act['nums'],null,false,true);
                }
            }

            if($goods_arr['store'] < $act['nums']){
                $this->end(false,'商品库存不能小于平台设置最低库存'.$act['nums'],null,false,true);
            }
        }else{
            $this->end(false,'活动不存在',null,false,true);
        }
        ...
        $object = kernel::single('groupbuy_business_activity');
        $rs = $object->addBusinessActivity($data);
        if($rs){
           $goods['act_type'] = 'group';
