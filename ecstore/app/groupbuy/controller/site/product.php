...
class groupbuy_ctl_site_product extends b2c_ctl_site_product{
    ...
    public function index() {
       ...
       $this->pagedata['specShowItems'] =$specValue;
        ...
        if ($aGoods['product']){ //如果商品有货品处理价格
            ...
            //修改团购价格
            //if($aGoods['act_type'] == 'group'){
                $groupapplyObj = $this->app->model('groupapply');
                $groupObj = $this->app->model('activity');
                $filter = array('gid'=>$this->id,'id'=>$act_id,'status'=>'2');
                $groupapply = $groupapplyObj->dump($filter,'*');
                //无活动时跳转至普通商品
                if(!$groupapply || empty($groupapply)){
                    header('Location:'.$normalG_url);
                    exit;
                }
                $activity = $groupObj->dump(array('act_id'=>$groupapply['aid']),'start_time,end_time,act_open,price_tag,activity_tag');

                $this->pagedata['activity'] = $activity;
                $this->pagedata['actapply'] = $groupapply;
                $nowTime = time();
                ...
                $aGoods['current_price'] = $groupapply['last_price'];
            //}
        ...
        $this->pagedata['goodshtml']['store'] = kernel::single('groupbuy_goods_detail_store')->show($gid,$aGoods);
        ...
        //页面基本信息  servicelist  
        $this->pagedata['info_page_list'] = $this->_get_servicelist_by('group_products_index_info');
        ///按钮  servicelist 
        $this->pagedata['btn_page_list'] = $this->_get_servicelist_by('group_products_index_btn');
    ...
    function joingroup(){
        $this->page('site/product/joingroup.html',true);
    }
