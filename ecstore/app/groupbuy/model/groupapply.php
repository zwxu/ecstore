<?php
class groupbuy_mdl_groupapply extends dbeav_model{
    function getOnActIdByGoodsId($goods_id){
        $nowTime = time();
        $sql = "select a.id as act_id from sdb_groupbuy_groupapply as a".
    ...
    function getOnActGIdByStatus($status){
        $nowTime = time();
        $sql = "select a.gid from sdb_groupbuy_groupapply as a".
        " join sdb_groupbuy_activity as s on s.act_id=a.aid".
    ...
    function loadActInfoById($id){
        ...
        "from sdb_groupbuy_groupapply as a".
        " join sdb_groupbuy_activity as s on s.act_id=a.aid".
        " where a.id={$id}";
        $result = $this->db->selectrow($sql);
