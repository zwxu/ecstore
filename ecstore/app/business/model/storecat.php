<?php
class business_mdl_storecat extends dbeav_model{
    ...
    public function getTree(){
        return $this->db->select('SELECT o.cat_name AS text,o.cat_id AS id,o.parent_id AS pid,o.p_order,o.cat_path,
                    o.type_id as type,o.child_count  FROM sdb_business_storecat o
    ...
    function cat2json($return=false){
        ...
        base_kvstore::instance('business_store')->store('store_cat.data',$contents);
    ...
    function get_cat_list($show_stable=false){
        ...
        if( base_kvstore::instance('business_store')->fetch('store_cat.data', $contents) !== false ){
    ...
    function toRemove($catid,&$msg=''){
		...
		$aStore = $obj_store->getList('store_id',array('store_cat'=>intval($catid),'disabled'=>'false'));
        if(count($aStore) > 0){
            $msg = '删除失败：本分类下面还有店铺';
            return false;
        }
        ...
        $this->db->exec('DELETE FROM sdb_business_storecat WHERE cat_id='.intval($catid));
        $this->db->exec('UPDATE sdb_business_storecat SET child_count = child_count-1 WHERE cat_id='.intval($parent_id));
        $this->cat2json();
        return true;
    }

    function get_new_cat($limit){
        $cat_id = $this->db->select('SELECT store_cat as cat_id FROM `sdb_business_storemanger`  where cat_id <> \'0\' group by store_cat order by store_id desc limit 0,'.$limit);
        if(is_array($cat_id)){
