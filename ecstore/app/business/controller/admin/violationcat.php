...
class business_ctl_admin_violationcat extends desktop_controller{
    /*
        %1 - id
        %2 - title
        $s - string
    ...
    */
    ...
    function index(){ 
        ...
        $objCat = &$this->app->model('violationcat');
        ...
        if($tree){
            foreach($tree as $k=>$v){
                $tree[$k]['link'] = array('cat_id'=>array(
                                'v'=>$v['cat_id'],
                                't'=>app::get('business')->_('违规类型').app::get('business')->_('是').$v['cat_name']
                            ));
            }
        }
        ...
        $this->page('admin/store/violation/map.html');
    }
    ...
    function _info($id=0,$type='add'){
         ...
         //   $this->pagedata['gtypes'] = $oGtype->getList('type_id,name');
           // $this->pagedata['gtype']['status'] = $oGtype->checkDefined();
            ...
            if($type == 'edit'){
                ...
                $this->pagedata['cat']['score'] = $aCat['score'];
        ...
        $this->display('admin/store/violation/info.html');
    }

     function save(){
         $this->begin('index.php?app=business&ctl=admin_violationcat&act=index');
         if( $_POST['p_order'] === '' )
             $_POST['p_order'] = 0;
    ...
    //     error_log(var_export($list,true),3,'c:/dd.txt');
        ...
        $this->display('admin/store/violation/cat_list.html');
