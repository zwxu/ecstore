    ...
    public function generate($sdf, &$controller=null, &$msg='')
    {
        ...
        switch($sdf_order['order_type']){
            ...
            case 'spike':
                ...
                if($apply['nums'] != ''){
                    $goodsnum = $orderItemObj->dump(array('goods_id'=>$apply['gid']),'(nums-sendnum) as gnum');
                    $sql = "update sdb_spike_spikeapply set remainnums=".($goodsnum['gnum']+$apply['remainnums']).' where id='.$sdf_order['act_id'];
                    $applyObj->db->exec($sql);
             ...
             case 'score':
                ...
                if($apply['nums'] != ''){
                    $goodsnum = $orderItemObj->dump(array('goods_id'=>$apply['gid']),'(nums-sendnum) as gnum');
                    $sql = "update sdb_scorebuy_scoreapply set remainnums=".($goodsnum['gnum']+$apply['remainnums']).' where id='.$sdf_order['act_id'];
                    $applyObj->db->exec($sql);
