...
class b2c_order_create extends b2c_api_rpc_request
{
    ...
    {	
        ...
        $sdf_order = array(
            'payment'=>$sdf['payment']['pay_app_id'],
            'shipping_id'=>$sdf['delivery']['shipping_id'],
            'is_protect'=>($sdf['delivery']['is_protect'][$sdf['delivery']['shipping_id']] ? $sdf['delivery']['is_protect'][$sdf['delivery']['shipping_id']] : 0),
            'currency'=>$sdf['payment']['currency'],
            'is_tax'=>($sdf['payment']['is_tax'] ? $sdf['payment']['is_tax'] : 'false'),
            'tax_company'=>($sdf['payment']['tax_company'] ? $sdf['payment']['tax_company'] : ''),
            'area'=>$sdf['delivery']['ship_area'],
            'split_order'=>$sdf['split_order']
            
        );
		/** 积分抵扣值的转换 **/
		$sdf_order['dis_point'] = $sdf['payment']['dis_point'];
		$sdf_order['member_id'] = $sdf['member_id'];
		$sdf_order['split_order'] = $sdf['split_order'];
		// 订单创建过程添加项埋点
        ...
        $order_detail = $obj_total->payment_split_detail($this->app->controller('site_order'),$objCarts,$sdf_order);
        // 订单显示方式        
		...
		//echo '<pre>';print_r($objCarts);exit;
		...
		//	echo '<pre>';print_r($objCarts['promotion']);exit;
                        ...
                        foreach ($arrObjInfo as $arrObjItems)
                        {
                            ...
                            if ($arrObjItems['obj_items']['products'][0]['package_use'] == '1')
                            {
                                ...
                                {                            
                                    ...
                                    {
                                        $temp = $arrObjItems['obj_items']['products'][0]['spec_desc']['spec_value'][$spec_key];// by cam
