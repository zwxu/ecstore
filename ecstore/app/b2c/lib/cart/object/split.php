...
class b2c_cart_object_split
{
    private $arr_parea=array();
    private $arr_area=array();
    public function split_order(&$controller,$area_id=null,&$sdf_cart){
        if(!isset($area_id)){
            //return;
        }
        $dt_id=array();
        //取得所有购物车类型对应的配送方式。
        foreach( kernel::servicelist('b2c_cart_object_split') as $object ) {
            if( !is_object($object) ) continue;
            if( method_exists($object,'get_dlytype') && is_callable(array($object,'get_dlytype')) ) {
               $temp_dt_id=$object->get_dlytype($sdf_cart);
        ...
        //取得结算单中所有的配送方式（不分店铺）。
        $all_dly_types=$this->get_dlytpye($controller,$dt_id,$area_id); 
        ...
        foreach( kernel::servicelist('b2c_cart_object_split') as $object ) {
            ...
            if( method_exists($object,'split_cart_object') && is_callable(array($object,'split_cart_object')) ) {
                $object->split_cart_object($sdf_cart,$split_dly,$valite_dt_id,$index);
            }
        }
        $mdl_business_store = app::get('business')->model('storemanger');
        $store_ids=array_keys($split_dly);
        $storeinfo = $mdl_business_store->getList('store_id,store_name,shop_name',array('store_id'=>$store_ids));
        $storeinfo=utils::array_change_key($storeinfo,'store_id');
        //每一张分单中的商品重量。
        foreach($split_dly as $store_id=>$sgoods){
            $split_dly[$store_id]['info']=$storeinfo[$store_id];
        ...
        $this->select_delivery_method($controller,$area_id,$sdf_cart,'',$all_dly_types);
                ...
                if(!empty($order['dly_type'])){
                    foreach($order['dly_type'] as $dkey=>$type_id){
                       ...
                       $temp_dly[$type_id]=array(
                           ...
                           'protect'=>$all_dly_types[$type_id]['protect'],
        ...
        //echo '<pre>';   print_r($sdf_cart['object']['goods']);echo '</pre>';
        //echo '<pre>';   print_r($split_dly);echo '</pre>';
        $sdf_cart['order_split']=$split_dly;
        $sdf_cart['json_order_split']=serialize($split_dly);
        $sdf_cart['all_dly_types']=$all_dly_types;
        return $split_dly;
    }
    
    public function select_delivery_method(&$controller, $area_id='', &$sdf_cart, $shipping_method='',&$all_dly_types)
    {   
    ...
    private function get_dlytpye(&$controller,$dt_id=array(),$area_id=null){
        if(empty($dt_id)){
            return array();
        }
