...
class b2c_cart_object_coupon implements b2c_interface_cart_object{
    ...
    public function _get($aData,$store_id=""){
    ...
    public function getAll($rich = false,$aData=array()) {
	   ...
	   if($aData){	  
		   $aData = array_values((array)$aData);
		   $goods_id = explode('_',$aData[0]);
		   $store_id = app::get('b2c')->model('goods')->getList('store_id',array('goods_id'=>$goods_id[1]));
		   $store_id = $store_id[0]['store_id'];
	   }//echo '<pre>';print_r($aData[0]);exit;
	   //end
        if(kernel::single("b2c_cart_object_goods")->get_cart_status()) {
        ...
        return $this->_get($aResult,$store_id);
    }
