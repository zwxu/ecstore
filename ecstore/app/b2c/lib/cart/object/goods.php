...
class b2c_cart_object_goods implements b2c_interface_cart_object{
	...
	private function _check_products_with_add($goods_id, $product_id, $quantity=0)
	{
		...
		if( !isset($this->check_goods_info[$goods_id]) )
            $this->check_goods_info[$goods_id] = $oSG->getList('goods_id, store,store_freeze,nostore_sell, marketable', array('goods_id'=> "$goods_id"));
        ...
        if ( !$aGoods['nostore_sell'] ) {
            ...
            //验证商品库存
            if(($aGoods['store'] - $aGoods['store_freeze'] - $quantity) < 0){
                return $this->get_error_msg( '该商品已无库存！' ); //有活动冻结库存，库存不足
            }
    ...
    // 购物车里的所有商品项
    public function getAll($rich = false, $aData = array()) {
        ...
        if($this->get_cart_status())  {
            ...
            if($aData){
                $filter['obj_ident'] = $aData;
            }
            $filter['member_ident'] = $this->member_ident;
            $filter['obj_type'] = 'goods';
    ...
    private function _get($aData) {
        ...
        $arr_products = array();
        $oGoods = $this->app->model('goods');// add bu lijun
        ...
        foreach($aData as $key => &$row) {
            ...
            $store_id = $oGoods->dump(array('goods_id'=>$goods_id),'store,store_freeze,store_id,freight_bear,nostore_sell');
            $aData[$key]['store_id'] = $store_id['store_id'];
            $aData[$key]['freight_bear'] = $store_id['freight_bear'];
            $aData[$key]['goods_kind'] = $arr_product_info['goods_kind'];
            unset($arr_product_info['goods_kind']);
            $aData[$key]['goods_kind_detail'] = $arr_product_info['goods_kind_detail'];
            unset($arr_product_info['goods_kind_detail']);
            ...
            if(isset($tmp_products_store[$product_id])) {
                ...
                $tmp_store = array(
                    ...
                    'real_store' => ( $store_id['nostore_sell'] ? $this->__max_goods_store : $store_id['store'] - $store_id['store_freeze'] ),//商品真实库存
                );
    ...
    private function get_products_real_store(&$tmp, &$products_store) {
         foreach($tmp as $val) {
            ...
            $products_store[$val['product_id']]['store'] = array(
                                                    'real' => min($val['store'],$val['real_store']) - $val['less'] + $val['quantity'],
                                                    'less' => $val['less'],
                                                    'store' => $val['store'],
                                                    'name' => $val['name'],
                                                );
    ...
    function _get_products($aProductId) {
        ...
        $sSql = "SELECT
                     ...
                     g.goods_type, g.nostore_sell, g.act_type,g.min_buy,g.type_id,g.cat_id,g.image_default_id,p.spec_info,p.spec_desc,p.price,p.weight,p.mktprice as p_mktprice,g.mktprice as g_mktprice,
                     t.setting, t.floatstore, g.goods_kind, g.goods_kind_detail
       ...
       foreach($aProduct as $row) {
           ...
           $aResult[$row['product_id']] = array(
                    ...
                    'goods_kind_detail' => $row['goods_kind_detail'],
           );
		   //组合JSON格式让JS显示
