 ...
 * $ 2010-05-04 17:06 $
 */
class b2c_promotion_solutions_bypercent implements b2c_interface_promotion_solution
{
    
    public $name = "减去固定折扣出售"; // 名称
    public $type = array('prefilter','cart'); // 应用范围 目前只有[预过滤(prefilter),购物车(cart)]
    public $desc_pre = '价格减去';
    ...
    public function config($aData = array()) {
        return <<<EOF
{$this->desc_pre}<input name="action_solution[b2c_promotion_solutions_bypercent][percent]" vtype='required&&unsigned' value="{$aData['percent']}" />{$this->desc_post}
EOF;
    }
    ...
    public function apply_order(&$object, &$aConfig,&$cart_object = null,$rule=array()) {
        ...
        if($info){
			$object['discount_amount_order'] += $omath->number_multiple( 
																		array( 
																			($info['subtotal']-$aCart['discount_amount_prefilter']),
																			$rulePercent/100
																		) 
																	);
		}else{
        ...
        $this->desc_pre = app::get('b2c')->_('订单总价格减去');
        $this->setString($aConfig);
    }
	...
	*/
	...
	public function verify_form($data=array(), &$msg='')
	{
		...
		/** 订单够满金额 **/
		if (!isset($data['action_solution']['b2c_promotion_solutions_bypercent']['percent']) || !$data['action_solution']['b2c_promotion_solutions_bypercent']['percent'])
		{
			$msg = app::get('b2c')->_('请指定订单优惠折扣的金额！');
			return false;
		}
		
		if (!is_numeric($data['action_solution']['b2c_promotion_solutions_bypercent']['percent']) || $data['action_solution']['b2c_promotion_solutions_bypercent']['percent'] < 0 || $data['action_solution']['b2c_promotion_solutions_bypercent']['percent'] > 100)
		{
			$msg = app::get('b2c')->_('折扣必须是在0-100之间的数字！');
			return false;
		}
		/** end **/
