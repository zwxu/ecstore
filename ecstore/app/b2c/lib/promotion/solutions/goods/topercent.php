 ...
 * $ 2010-05-04 17:06 $
 */
class b2c_promotion_solutions_goods_topercent implements b2c_interface_promotion_solution
{
    ...
    public function config($aData = array()) {
        return <<<EOF
        {$this->desc_pre}<input name="action_solution[b2c_promotion_solutions_goods_topercent][percent]" vtype='required&&unsigned' value="{$aData['percent']}" />{$this->desc_post}
EOF;
    }
    ...
    public function apply(&$object,$aConfig,&$cart_object = null) {
        ...
        if(is_null($cart_object)) { // 商品预过滤
            $object['obj_items']['products'][0]['price']['buy_price'] *= (1 - ($rulePercent / 100)) ;
        } else {// 购物车里的处理
