...
class b2c_api_ocs_1_0_goods
{
    ...
    public function get_all_list(&$sdf, &$thisObj)
    {
		...
		//增加时间范围
		if ($sdf['start_time'])
            $condition .= " AND last_modify >=".$sdf['start_time'];
		if ($sdf['end_time'])
            $condition .= " AND last_modify <=".$sdf['end_time'];

        //增加按商品编号
        if($sdf['search_bn'])
             $condition .= " AND bn in (".$sdf['search_bn'].")";
		...
		//计算总个数
		$total_results = $db->count("SELECT * FROM `sdb_b2c_goods` WHERE 1".$condition." LIMIT ".$page_no.",".$page_size);
        ...
        return array('total_results'=>$total_results,'items'=>json_encode($sdf_goods));
    }
    ...
    private function get_item_detail($arr_row, $obj_clt_goods)
    {
        ...
        if ($arr_skus)
        {
            foreach ($arr_skus as $arr)
            {
                ...
                $arr_sdf_skus[] = array(
                    ...
                    'status'=>$arr['marketable'],
                    'modified'=>$arr['last_modify'],
                    'cost'=>$arr['cost'],
					'name'=>$arr['name'],
					'spec_info'=>$arr['spec_info'],
					'spec_desc'=>$arr['spec_desc'],
                );
            }
        ...
        if(!empty($arr_row['goods_id'])){
            $image_arr=array();
            $image_attach_arr=array();
            $image_attach_arr=$mdl_image_attach->getList('target_id,target_type,image_id,last_modified',array('target_type'=>'goods','target_id'=>$arr_row['goods_id']));
            $image_id_arr=array();
            $image_id_arr[]=$arr_row['image_default_id'];
            foreach($image_attach_arr as $att_k=>&$att_v){
                $image_id_arr[]=$att_v['image_id'];
                $att_v['image_id']=md5('b2b2c'.$att_v['image_id']);
            }
            $image_id_arr=array_unique($image_id_arr);
            $image_arr=$mdl_image->getList('*',array('image_id|in'=>$image_id_arr));
            foreach($image_arr as $img_k=>&$img_v){
                    $img_v['ident']=base_storager::image_path($img_v['image_id']);
                    $img_v['url']=base_storager::image_path($img_v['image_id']);
                    $img_v['l_ident']=base_storager::image_path($img_v['image_id'],'l');
                    $img_v['l_url']=base_storager::image_path($img_v['image_id'],'l');
                    $img_v['m_ident']=base_storager::image_path($img_v['image_id'],'m');
                    $img_v['m_url']=base_storager::image_path($img_v['image_id'],'m');
                    $img_v['s_ident']=base_storager::image_path($img_v['image_id'],'s');
                    $img_v['s_url']=base_storager::image_path($img_v['image_id'],'s');
                    $img_v['storage']='network';
                $img_v['image_id']=md5('b2b2c'.$img_v['image_id']);
            }
        }
        ...
        return array(
            ...
            'image_default_id'=>$arr_row['image_default_id']?md5('b2b2c'.$arr_row['image_default_id']):NULL,
			...
			'spec_desc' => $arr_row['spec_desc'],
            'skus'=>$str_skus,
            'url'=>'product-'.$arr_row['goods_id'].'.html',
            'syn_type'=>'1',
            'image_arr'=>$image_arr,
            'image_attach_arr'=>$image_attach_arr,
            'marketable'=>$arr_row['marketable'],
            'uptime'=>$arr_row['uptime'],
            'downtime'=>$arr_row['downtime'],
            'p_1'=>$arr_row['p_1'],
            'p_2'=>$arr_row['p_2'],
            'p_3'=>$arr_row['p_3'],
            'p_4'=>$arr_row['p_4'],
    ...
    public function get_types(&$sdf, &$thisObj)
    {
		...
		$totalResults = $db->count("SELECT * FROM `sdb_b2c_goods_type` LIMIT ".$page_no.",".$page_size."");
        if (!$arr_goods_types)
            return array();
        ...
        foreach ($arr_goods_types as $index=>$arr)
        {
            ...
            /** properties array **/
            $arr_type_props = $db->select("SELECT * FROM `sdb_b2c_goods_type_props` WHERE `type_id`=".$arr['type_id']);
            $arr_prop_props_ids = array();
            $arr_prop_names = array();
            $arr_prop_types = array();
            $arr_prop_options = array();
            $arr_prop_value_ids = array();
            $arr_prop_value_arr=array();
            foreach ($arr_type_props as $arr_prop)
            {
                ...
                $arr_props_value = $db->select("SELECT * FROM `sdb_b2c_goods_type_props_value` WHERE `props_id`=".$arr_prop['props_id']);
                $arr_prop_value_arr=array_merge($arr_props_value,$arr_prop_value_arr);
            ...
            $arr_sdf_goods_types[$index]['props'] = array(
                ...
                'props_arr'=>$arr_type_props,
                'props_value_arr'=>$arr_prop_value_arr
            );
            /** specialist array **/
            ...
            $type_ids[]=$arr['type_id'];
        }
        $arr_type_brands= $db->select("SELECT * FROM `sdb_b2c_type_brand` where type_id in (".implode(',',$type_ids).')');

        return array('datas'=>$arr_sdf_goods_types,'totalResults'=>$totalResults,'arr_type_brands'=>$arr_type_brands);
    }
    ...
    public function get_cat(&$sdf, &$thisObj)
    {
        ...
        $condition = "";
        if ($sdf['cat_name'])
            $condition .= " AND cat_name='".$sdf['cat_name']."' ";

        $page_no = intval($sdf['page_no']) - 1;
        $page_size = intval($sdf['page_size']);

        $db = kernel::database();
        $arr_goods_cats = $db->select("SELECT * FROM `sdb_b2c_goods_cat` WHERE 1".$condition." LIMIT ".$page_no.",".$page_size."");
		$totalResults = $db->count("SELECT * FROM `sdb_b2c_goods_cat` WHERE 1".$condition." LIMIT ".$page_no.",".$page_size."");
        ...
        return array('datas'=>$sdf_arr_goods_cats,'totalResults'=>$totalResults);
    }
    ...
    public function get_brands(&$sdf, &$thisObj)
    {
        ...
        $condition = "";
        if ($sdf['brand_name'])
            $condition .= " AND brand_name='".$sdf['brand_name']."' ";
        $page_no = intval($sdf['page_no']) - 1;
        $page_size = intval($sdf['page_size']);

        $db = kernel::database();

        $arr_goods_brands = $db->select("SELECT `brand_id`,`brand_name`,`brand_url` FROM `sdb_b2c_brand` WHERE 1".$condition." LIMIT ".$page_no.",".$page_size."");
		$totalResults = $db->count("SELECT `brand_id`,`brand_name`,`brand_url` FROM `sdb_b2c_brand` WHERE 1".$condition." LIMIT ".$page_no.",".$page_size."");
        if (!$arr_goods_brands)
            return array();
        ...
        return array('datas'=>$sdf_arr_goods_brands,'totalResults'=>$totalResults);
    }
    ...
    public function get_specs(&$sdf, &$thisObj)
    {
		...
		$totalResults = $db->count("SELECT * FROM `sdb_b2c_specification` LIMIT ".$page_no.",".$page_size."");
        ...
        return array('datas'=>$sdf_arr_goods_specifications,'totalResults'=>$totalResults);
    }


   //公共平台同步 返回b2b2c里的商品 chenlong 2013-6-3
    function get_b2b2c_goods($request,$rpcService){
        $tableprefix= kernel::database()->prefix;
        $router=app::get('site')->router();
        $filter=array();
        if($request['bn'])
            $filter['bn']=$request['bn'];
        if($request['start_time'])
            $filter['last_modify|bthan']=$request['start_time'];
        if($request['end_time'])
             $filter['last_modify|sthan']=$request['end_time'];
        ...
        $goods_id_arr=array();
        $products=array();
        $goods=$mdl_goods->getList('*',$filter,$offset,$limit,'goods_id asc ');
        $filter=array();
        foreach($goods as $key=>&$value){
            //$goods_id_arr[]=$value['goods_id'];
            //$value['category']=array('cat_id'=>$value['cat_id']);
            $value['type']=array('type_id'=>'1');
            //$value['brand']=array('brand_id'=>$value['brand_id']);
            $value['goods_setting']=array('goods_template'=>$value['goods_template']);
            //$value['images'][0]=array('target_type'=>'goods','');
            $value['status']=$value['marketable'];//上架
            $value['gain_score']=$value['score'];//积分
            $value['description']=$value['intro'];//详细介绍
            $value['props']=array();
            for($i=1;$i<=50;$i++){
                if($value['p_'.$i]!=''){
                    $value['props']['p_'.$i]=array('value'=>$value['p_'.$i]);
                }
            }
            $value['url']=$router->gen_url( array('app'=>'b2c','ctl'=>'site_product','act'=>'index','arg0'=>$value['goods_id'],'full'=>'true') );
            $value['image_default_id']=$value['image_default_id']?md5('b2b2c'.$value['image_default_id']):NULL;
            $value['syn_type']='1'; //syn_type '0'=>'b2c','1'=>'b2b2c'
            unset($value['spec_desc']);
        }
        if(!empty($goods_id_arr)){
            //同步图片
            /*
            $image_sql='SELECT image.* from '.$tableprefix.'image_image as image inner join '
                      .$tableprefix.'image_image_attach as attach on image.image_id=attach.image_id where attach.target_type=\'goods\' '
                      .' and attach.target_id in ('.implode(',',$goods_id_arr).') ';

            $image_arr=$mdl_image->getList($image_sql);
            */


            $filter['goods_id|in']=$goods_id_arr;
            $products=$mdl_products->getList('*',$filter);
            foreach($goods as $key=>&$value){
                $image_arr=array();
                $image_attach_arr=array();
                $image_attach_arr=$mdl_image_attach->getList('target_id,target_type,image_id,last_modified',array('target_type'=>'goods','target_id'=>$value['goods_id']));
                $image_id_arr=array();
                foreach($image_attach_arr as $key=>&$value){
                    $image_id_arr[]=$value['image_id'];
                    $value['image_id']=md5('b2b2c'.$value['image_id']);
                }

                $image_arr=$mdl_image->getList('*',array('image_id|in'=>$image_id_arr));
                foreach($image_arr as $key=>&$value){
                        $value['ident']=base_storager::image_path($value['image_id']);
                        $value['url']=base_storager::image_path($value['image_id']);
                        $value['l_ident']=base_storager::image_path($value['image_id'],'l');
                        $value['l_url']=base_storager::image_path($value['image_id'],'l');
                        $value['m_ident']=base_storager::image_path($value['image_id'],'m');
                        $value['m_url']=base_storager::image_path($value['image_id'],'m');
                        $value['s_ident']=base_storager::image_path($value['image_id'],'s');
                        $value['s_url']=base_storager::image_path($value['image_id'],'s');
                        $value['storage']='network';
                    $value['image_id']=md5('b2b2c'.$value['image_id']);
                }
                $value['image']['attach']=$image_attach_arr;
                $value['image']['image']=$image_arr;
