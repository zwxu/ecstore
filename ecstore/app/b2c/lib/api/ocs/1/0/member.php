...
class b2c_api_ocs_1_0_member {
    ...
    */
	...
	//为会员添加收货地址 yindingsheng
	function member_addr_add($request, $rpcService){

		if ( ! $request['foreign_id'] || !$request['member_foreign_id']) {
            $rpcService->send_user_error(app::get('b2c')->_('重要参数缺失'));
        }

		$foreign_id = $request['member_foreign_id'];
		$memberMdl = $this->app->model('members');
        $member = $memberMdl->getList('member_id',array('foreign_id'=>$foreign_id),0,1);
		...
		$nMemberId = $member[0]['member_id'];

		$aData = $request;

		if($request['phone']){
			$aData['phone'] = json_decode($request['phone'],1);
		}
		...
		if($objMemberAddr->is_exists_addr($aData,$nMemberId)){
            $rpcService->send_user_error(app::get('b2c')->_('地址重复'));
        }

		if($objMemberAddr->save($aData)){
            return $objMemberAddr->db->lastInsertId();
        }else{
            $rpcService->send_user_error(app::get('b2c')->_('添加失败'));
        }
	}

	function member_addr_edit($request, $rpcService){
...
//error_log(var_export($aData,true),3,'c:/dd.txt');
		$objMemberAddr = $this->app->model('member_addrs');
        $row = $objMemberAddr->getList('addr_id',array('member_id' => $nMemberId,'foreign_id' => $aData['foreign_id']));

		if($row){
			$aData['addr_id'] = $row[0]['addr_id'];
		}else{
			$rpcService->send_user_error(app::get('b2c')->_('找不到该地址，编辑失败'));
		}
	...
	function member_addr_delete($request, $rpcService){
		...
		$filter['member_id'] = $nMemberId;
		$member_addr = app::get('b2c')->model('member_addrs');

		if($member_addr->delete($filter)){
			return true;
		}else{
			$rpcService->send_user_error(app::get('b2c')->_('删除失败'));
		}
