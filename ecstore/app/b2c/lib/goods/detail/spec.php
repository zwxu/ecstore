...
//            $this->pagedata['SelectSpecValue'] = array('value'=>implode("、",(array)$specValue),'selected'=>0);
        ...
        $act_id = $timed_bus_obj->getList('remainnums',array('gid'=>$gid,'disabled'=>'false','status'=>'2'));
        if(isset($act_id[0]['remainnums'])){
            $remainnums = $act_id[0]['remainnums'];
        }
        /********-------------------*********/
        ...
        foreach($aGoods['product'] as $product_id=>$product){
            ...
            if(isset($remainnums)){
                $store = min($remainnums,min(($goods_real_store-$freeze_total),(isset($product['store'])?($product['store']-intval( $product['freez'] )):999999 )));
            }else{
                $store = min(($goods_real_store-$freeze_total),(isset($product['store'])?($product['store']-intval( $product['freez'] )):999999 ));
            }
